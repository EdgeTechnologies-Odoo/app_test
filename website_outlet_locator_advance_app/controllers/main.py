...
class DynamicController(http.Controller):
		...
		if store_id.google_api_key:
			...
			for location_id in location_ids:
				...
				location_vals = {
						...
						'web_address' : location_id.web_address,
					}
				categ_vals = []
				for categ in location_id.category_id:
					categ_vals.append(categ.name)
				...
				location.update({
					location_id.id : {
						...
						'map_search_radius' : store_id.search_radius,
						'web_address' : location_id.web_address,
						'loc_category' : categ_vals if categ_vals else False,
					}
				})
			...
			categs = request.env['store.category'].sudo().search([])
			cat = []
			for c in categs:
				cat.append(c.name)
			data.update({
	...
	@http.route(['/category/data'] , type='json', auth="public", website=True)
	def store_locator(self, **post):
		print("SSSSSSSSSSSSss",post)
		categ_value = []
		categs = request.env['store.category'].sudo().search(post['domain'])
		for categ in categs:
			category = []
			category.append(categ.id)
			category.append(categ.name)
			categ_value.append(category)
		print("AAAAAAAAAAAAAAAAAA",categ_value)
		return {
			'categs': categ_value
		}
	...
	def search_vals(self, **post):
		...
		for location_id in location_ids:
			...
			if location_id.category_id:
				categ_name = []
				for categ in location_id.category_id:
					categ_name.append(categ.name)
					category[location_id.id] = categ_name
			...
			if 'categs' in post and post['categs']:
				for name in category:
					if len(set(category[name]).intersection(post['categs'])):
						location.update({
							location_id.id : {
								'name' : location_id.name,
								'contact' : location_id.contact,
				...
				if not location_id.category_id:
					location.update({
						location_id.id : {
							'name' : location_id.name,
							'contact' : location_id.contact,
